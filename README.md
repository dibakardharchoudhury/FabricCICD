...
One notebook — `NB_05_Deploy` — publishes every item in this Git repo into a target
...
| `NB_05_Deploy.Notebook/` | **The deploy tool** — publishes everything above to a target workspace |
| `parameter.yml` | fabric-cicd binding rules (auto-generated by `NB_05_Deploy`) |
...
`NB_05_Deploy` does three things in one run:
...
- A target workspace (e.g. `ws-CICD-PROD`) — it can be empty; NB_05 creates the lakehouse shells too.
- The identity running NB_05 is **Admin/Member on both workspaces** and **owns `Gold_SM`**
  (required to rebind Direct Lake).

**Steps**

1. Open `NB_05_Deploy` in a Fabric workspace (it deploys from the repo, including itself).
...
3. **Run all cells.** NB_05 publishes every item, repoints references to the target stage,
...
- **Direct Lake on OneLake** can't be rebound by a deployment rule, so NB_05 does it in

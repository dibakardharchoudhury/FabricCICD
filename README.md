...
2. Add the GitHub OIDC federated credential in **Microsoft Entra ID → App registrations →
   fabric-rest → Certificates & secrets → Federated credentials → Add credential**. Complete the
   current **Add a credential** form as follows:

   | Form field | Value |
   | --- | --- |
   | Federated credential scenario | **GitHub Actions deploying Azure resources** |
   | Issuer | Leave the auto-populated `https://token.actions.githubusercontent.com` value |
   | Organization | `dibakardharchoudhury` |
   | Organization ID | `1668148` |
   | Repository | `FabricCICD` |
   | Repository ID | `1270097880` |
   | Entity type | **Environment** |
   | GitHub environment name | `production` |
   | Name | `github-fabriccicd-production` |

   After selecting **Environment** and entering `production`, verify the read-only **Subject
   identifier** generated by Entra contains the same organization, organization ID, repository,
   repository ID, and environment. Do not replace it with the legacy name-only subject
   `repo:dibakardharchoudhury/FabricCICD:environment:production`; the current form binds the trust to
   immutable GitHub IDs. The environment name must match the workflow's `environment: production`

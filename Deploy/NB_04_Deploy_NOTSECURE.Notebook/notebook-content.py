...
# ⚠️ TEMP ONLY — HARDCODED GITHUB PAT. This file is a throwaway copy of NB_04_Deploy for quick runs
#   where Key Vault is not yet wired up. SECURITY: do NOT commit this with a real token, revoke the
#   PAT right after use, use a FINE-GRAINED, READ-ONLY (Contents: Read) PAT scoped to just this repo,
#   and DELETE this NB_04_Deploy_TEMP notebook when done. When set (non-empty), Cell 4 uses this value
#   directly and SKIPS the Key Vault lookup. Leave "" to fall back to Key Vault (git_pat_secret).
github_pat     = "ghp_REPLACE_WITH_YOUR_GITHUB_PAT"   # ← paste your PAT here for the temp run
...
if local_repo_path and local_repo_path != "SKIP_AUTODETECT":
    ...
    if _auto:
        ...
        # which has no working tree). PAT comes from the hardcoded github_pat if set, else Key Vault.
        _pat = github_pat or notebookutils.credentials.getSecret(key_vault_url, git_pat_secret)
